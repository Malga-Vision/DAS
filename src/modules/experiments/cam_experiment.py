...
class CAMExperiment(Experiment):
    ...
    def run_config(self, params, N, eta_G, eta_H):
        ...
        for k in range(self.num_tests):
            ...
            A_SCORE, top_order_SCORE, SCORE_time, tot_time =  SCORE(X, eta_G, eta_H, cam_cutoff=cam_cutoff, pruning="CAM")
            fn, fp, rev, SHD, SID, top_order_errors = self.metrics(A_SCORE, adj, top_order_SCORE, compute_SID)
            pretty_evaluate(self.pruning, None, adj, A_SCORE, top_order_errors, SCORE_time, tot_time, compute_SID)
